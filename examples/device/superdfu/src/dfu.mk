...
$(BUILD)/$(DFUED_ELF_FILE): $(BUILD)/$(ELF_FILE)
	@echo CREATE $@
	@$(CP) $^ $@.tmp
	$(PYTHON) $(TOP)/examples/device/superdfu/src/superdfu-patch.py --strict 1 --tag $(BUILD)/$(DFU_TAG_FILE) $@.tmp $(shell $(OBJDUMP) -t $^ | grep _sfixed | head -n 1 | awk '{print $$1;}') $(shell $(OBJDUMP) -t $^ | grep dfutag | head -n 1 | awk '{print $$1;}')
	@mv $@.tmp $@
...
$(BUILD)/$(DFUED_BIN_FILE): $(BUILD)/$(DFUED_ELF_FILE)
	@echo CREATE $@
	@$(OBJCOPY) -O binary $^ $@
	#$(PYTHON) $(TOP)/examples/device/superdfu/src/crc32.py 0 $(shell $(OBJDUMP) -t $^ | grep dfutag | head -n 1 | awk '{print $$1;}') $@
